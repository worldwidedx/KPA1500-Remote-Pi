...
const controls = { operate: '^OS1;', standby: '^OS0;', powerOn: '^ON1;', powerOff: '^ON0;', tune: '^FT;', cancelTune: '^FE;' };
...
async function remoteApi(req, res, pathname) {
  ...
  if (pathname === '/api/control' && req.method === 'POST') { const data = await body(req); let command = controls[data.action]; if (data.action === 'antenna' && Number.isInteger(data.value) && data.value >= 1 && data.value <= 32) command = `^AN${data.value};`; if (!command) return json(res, 400, { error: 'Unsupported control' }); try { amp.send(command); return json(res, 202, { ok: true }); } catch (error) { return json(res, 503, { error: error.message }); } }
